...
if ! docker exec -it bitcoind-node bitcoin-cli getblockchaininfo; then
  echo "ERROR: failed to start bitcoind node"
  exit 1
fi

# query the lightclient for the latest block
chaintip_response_before=$(sui client call --function latest_block_hash --module light_client --package $PACKAGE_ID --gas-budget 10000000 --args $LIGHT_CLIENT_ID --dev-inspect --json)
...
chaintip_response_after=$(sui client call --function latest_block_hash --module light_client --package $PACKAGE_ID --gas-budget 10000000 --args $LIGHT_CLIENT_ID --dev-inspect --json)
